...
NUM_CLASSES = 103
checkpt = '/gpfs/data/luilab/karthik/pediatric_seg_proj/results_copied_from_kn2347/pooled_labels_ckpt_continue_8-22-23/model_best_20230822-115028.pth'
# %% set up model
medsam_model = build_sam_vit_b_multiclass(num_classes=max(NUM_CLASSES, 3), checkpoint=checkpt)
...
class Window(QWidget):
    ...
    def mouse_release(self, ev):
        ...
        box_1024 = box_np / np.array([W, H, W, H]) * 256
...
dfdf = pd.read_csv('/gpfs/data/luilab/karthik/pediatric_seg_proj/path_df_constant_bbox_remove_most_blank.csv')
rnd_embed_path = dfdf.sample(n=1)['image_embedding_slice_path']
print(rnd_embed_path)
img_embed_npy = np.load(rnd_embed_path) # (256, 64, 64)
img_embed_tensor = torch.from_numpy(img_embed_npy).cuda()
print(img_embed_tensor.shape)
w = Window(embedding = img_embed_tensor)
w.show()
