...
export label=$1
export frac=$2
export workdir='/gpfs/data/luilab/karthik/pediatric_seg_proj/results_copied_from_kn2347/unet_compare_blank_training_9-6-24/training'
mkdir -p ${workdir}/${label}/${frac}
if ls ${workdir}/${label}/${frac}/*.pth 1> /dev/null 2>&1; then 
    exit 1
fi
...
do
    ...
    --data_frame_path /gpfs/data/luilab/karthik/pediatric_seg_proj/per_class_isolated_df_new/${label}/${frac}/isolated_path_df_unet_FIXED.csv \
    ...
    -label_id ${label} -num_classes 1 -batch_size 64 -num_workers 2 \
    -lr .0001 \
    -epochs 5 \
    -work_dir ${workdir}/${label}/${frac} \
    -project_name singletask_unet -wandb_run_name label${label}_frac${frac}" >> ${workdir}/${label}/${frac}/log_for_${SLURM_JOB_ID}.log 2>&1 &
done
