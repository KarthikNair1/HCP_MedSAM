...
my_start = int((args.node_rank / total_nodes) * total_len)
my_end = int(((args.node_rank + 1) / total_nodes) * total_len)
if args.node_rank == total_nodes - 1:
        assert my_end == total_len
lab_nums = torch.Tensor(val_dataset.data_frame['label_number'].to_numpy()).long().cuda()[my_start:my_end]
print(lab_nums.shape)
val_dataset = torch.utils.data.Subset(val_dataset, range(my_start, my_end))
...
print(concat.shape)
np.save(f'/gpfs/data/luilab/karthik/pediatric_seg_proj/results_copied_from_kn2347/pooled_labels_ckpt_continue_8-22-23/val_dices_{args.node_rank}.npy', concat)
